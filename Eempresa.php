...
if ($result = $conexion->query($query)) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        // @Luis verificacon de boton actualizar
...
$rfc = $_POST["rfc"];
  ...
  // @Luis arreglo de errores
$error=array("nombre"=>"","telefono"=>"","country"=>"","direccion"=>"","descripcion"=>"","rfc"=>"");
   
     // @Luis validacion campo de nombre
   if($nombre=="")
    {
 ...
 // @Luis validacion campo de telefono
    if($telefono=="")
    {
...
if($pais=="")
 ...
 // @Luis validacion campo de direccion
    if($direccion=="")
    {
 ...
 // @Luis validacion campo de descripcion
    if($descripcion=="")
    {
 ...
 // @Luis validacion campo de rfc
    ...
    }else
    {
     ...
     // @Luis validacion de para actualizar datos

   if((strlen($valid_nom)>0)&& (strlen($valid_tel)>0)&&(strlen($valid_country)>0)&&(strlen($valid_dir)>0)&&(strlen($valid_des)>0)&&(strlen($valid_rfc)>0) )
{


 // @Luis sentencia para actualizar
   $upd="UPDATE empresa SET nombre='$nombre',telefono='$telefono',pais='$pais',direccion='$direccion',descripcion='$descripcion',rfc='$rfc' where usr='$usrm'";
...
<form name="f1" method="post"  enctype="multipart/form-data" >
  <table border="1" align="center" rules="groups" height="80%">
    <tr bgcolor="#000000" >
      <th align="left" colspan="2"><h1 style="color:#FFF;padding-left:20px">Edit Profile: <?php echo $usuario?></h1></th>
    </tr>


    <tr>
      <td width="21%">Name*</td>
      <td width="79%"><input type="text" name="nom" id="n" class="fieldsize" value="<?php echo $nom?>" />
...
<tr>
      ...
      <td width="79%"><input type="text" name="tel" id="n" class="fieldsize" value="<?php echo $tel?>" />
...
<tr>
      <td>Address*</td>
      <td><textarea name="dir" class="fieldsize"><?php echo $dir?></textarea>
...
<tr>
      <td width="21%">RFC*</td>
      <td width="79%"><input type="text" name="rfc" id="n" class="fieldsize" value="<?php echo $rfc?>" />
