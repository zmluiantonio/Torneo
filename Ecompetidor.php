<?php
  ...
  // @Luis Editar informacion de competidor
...
$error=array("nombre"=>"","country"=>"","mail"=>"","cta"=>"","paypal"=>"","rfc"=>"","dir"=>"");
...
if($mail=="")
    ...
    }else
    {
      ...
      if($banco=="")
    {
        $error['cta']="Campo requerido.";
    }else{

         if(!preg_match('/^[_0-9]+$/',$banco))
        {
        $error['cta']="Cuenta invalida";
        }
        else
        {
            if(strlen($banco)==10)
            ...
            {
            $error['cta']="La cuenta de banco es de 10 digitos";
            }
...
if($paypal=="")
   ...
   if((strlen($valid_nom)>0)&& (strlen($valid_country)>0)&&(strlen($valid_mail)>0)&&(strlen($valid_banco)>0)&&(strlen($valid_pp)>0)&&(strlen($valid_dir)>0)&& (strlen($valid_rfc)>0) )
{
    

   $upd="UPDATE competidor SET nombre='$nombre',pais='$pais',email='$mail',cta_bancaria='$banco',paypal='$paypal',rfc='$rfc',direccion='$dir' where usr='$usrm'";
  ...
  if($ejecutar){
    echo "<script>alert('Registro exitoso');
            window.location='competidor.php';
            </script>";}
   
 else
           { echo "<script>alert('Error al registrarse');
            window.location='Ecompetidor.php';
            </script>";;
                $conexion->close();
            }
...
<form name="f1" method="post"  enctype="multipart/form-data">
  <table border="1" align="center" rules="groups" height="150%">
    <tr bgcolor="#000000" >
      <th align="left" colspan="2"><h1 style="color:#FFF;padding-left:20px">Edit Profile:<?php echo $usrm?></h1></th></tr>


    
    <tr>
      <td width="21%">Name</td>
      <td width="79%"><input type="text" id="n" class="fieldsize" name="nom" value="<?php echo $nom;?>" />
...
<tr>
      ...
      <td><select name="country" class="fieldsize" >
       ...
       <span class="err"><?php echo $error["countryn"];?></span></td>
    </tr>


<tr>
      <td width="21%">Email</td>
      <td width="79%"><input type="text"  id="n" class="fieldsize" name="email" value="<?php echo $email;?>" />
       <span class="err"><?php echo $error["mail"];?></span></td>
    </tr>
<tr>
      <td width="21%">Cta_Bancariabank account</td>
      <td width="79%"><input type="text"  id="n" class="fieldsize" name= "cta"value="<?php echo $cta;?>"  />
      <span class="err"><?php echo $error["cta"];?></span></td>
    </tr>

    <tr>
      <td width="21%">Paypal</td>
      <td width="79%"><input type="text"  id="n" class="fieldsize" name="pp" value="<?php echo $paypal;?>"  />
       <span class="err"><?php echo $error["paypal"];?></span></td>
    </tr>

    <tr>
      <td width="21%">RFC</td>
      <td width="79%"><input type="text"  id="n" class="fieldsize" name="rfc" value="<?php echo $rfc;?>"  />
      <span class="err"><?php echo $error["rfc"];?></span></td>
    </tr>

<tr>
      <td width="21%">Address</td>
      <td width="79%"><textarea  class="fieldsize"  name="dir"><?php echo $direccion;?></textarea>
      <span class="err"><?php echo $error["dir"];?></span></td>
    </tr>
...
<tr>
      <td colspan="2" align="left" ><input type="submit" name="actualizar" value="Update" />
                                    <input type="button" value="Cancel" onclick="window.location='competidor.php';" />
