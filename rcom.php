...
$error=array("nombre"=>"","country"=>"","mail"=>"","cta"=>"","paypal"=>"","rfc"=>"","dir"=>"","usr"=>"","pw"=>"","cpw"=>"","file"=>"");
...
if($mail=="")
    {
        $error['mail']="campo requerido";
    }else
    {

   $consulta = "SELECT * FROM competidor WHERE email='$mail'";
...

    {
        ...
        {
            $error['mail']="Email invalido";
        }
   
   }else{
        $error['mail']="Ya existe el email";
    }
...
if($usr=="")
    ...
    { 
        ...
        $consulta = "SELECT * FROM competidor WHERE usr='$usr'";
    $econs = $conexion->query($consulta);
    $reng=$econs->num_rows;
    if($reng == 0)
    ...
    }else
    {
        ...
        {
            $error['file']="La imagen de ser en formato jpg.";   
        }
    }


   if((strlen($valid_nom)>0)&& (strlen($valid_country)>0)&&(strlen($valid_mail)>0)&&(strlen($valid_banco)>0)&&(strlen($valid_pp)>0)&&(strlen($valid_dir)>0)&& (strlen($valid_rfc)>0) && (strlen($valid_usr)>0) && (strlen($valid_pw)>0) && (strlen($valid_cpw)>0) && (strlen($valid_file)>0))
{
...
$cpw=$_POST["cpw"];
*/
$cinsertar = "INSERT INTO competidor (nombre,pais,email,cta_bancaria,paypal,RFC,direccion,usr,pasw,file) VALUES 
  ('$nombre','$pais','$mail','$banco','$paypal','$rfc','$dir','$usr','$pw','$content')";
...
<form name="f1" method="post"  enctype="multipart/form-data" >
  <table border="1" align="center" rules="groups" height="80%">
    <tr bgcolor="#000000" >
      <th align="left" colspan="2"><h1 style="color:#FFF;padding-left:20px">Registrar Competidor</h1></th>
    </tr>
...
<tr>
      <td>Email*</td>
      <td width="79%"><input type="text" name="mail" id="n" class="fieldsize" value="" />
        <span class="err"><?php echo $error["mail"];?></span></td>
    </tr>

<tr>
      <td>Cuenta Bancaria*</td>
     <td width="79%"><input type="text" name="cta" id="n" class="fieldsize" value="" />
      <span class="err"><?php echo $error["cta"];?></span><td/>
    </tr>

<tr>
      <td>Paypal*</td>
     <td width="79%"><input type="text" name="paypal" id="n" class="fieldsize" value="" />
